...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        ...
        $('#tb_departments').bootstrapTable({
            url: basePath+'/bk/l/sys/menuList',  //请求后台的URL（*）       
            method: 'get',                      //请求方式（*）
...
function update(id){
	retFrom();
	button_submit = "editMenuId";
	tmUtil.request({
		url:basePath+"/bk/l/sys/getMenuId",
		data:{
			"id":id
		},
		success:function(data){
			console.log(data);
			$("#id").val(data.id);
			$("#menuName").val(data.operName);
			$("#menuDesc").val(data.operDesc);
			$("#url").val(data.url);
			...
			if(data.isEnable==0){
				$("#isTrue2").click();
				$(".isTrue[value='0']").attr("checked","checked");
			}else{
				$("#isTrue1").click();
				$(".isTrue[value='1']").attr("checked","checked");
			}
		}
	});
	$("#myModal").modal('show');
...
function clearLeftMenu(){
	console.log("清空菜单")
	tmMap.remove(key,2);//清除sessionStorage存储
}
