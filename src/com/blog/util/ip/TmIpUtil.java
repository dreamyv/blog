...
import com.blog.util.method.WebPath;
...
public class TmIpUtil {
	...
	public static String ipLocation(String ip) {
		String ipLocation ="";
		String dir = WebPath.getProperty()+"\\WebContent\\WEB-INF\\plugin\\";
		System.out.println(dir);
		TmIPSeeker ipSeeker = new TmIPSeeker("qqwry.dat",dir);
	...
	public static String ipLocation(HttpServletRequest request) {
		...
		try{
			...
			String path = WebPath.getRealPath(request) + "/WEB-INF/plugin";
			if (StringUtil.isNotEmpty(path)) {
				path = StringUtil.conversionSpecialCharacters(path);
				TmIPSeeker ipSeeker = new TmIPSeeker("qqwry.dat", path);
				ipLocation = ipSeeker.getIPLocation(ip).getCountry() + " "
						+ ipSeeker.getIPLocation(ip).getArea();
			}
