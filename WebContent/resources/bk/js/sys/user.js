...
var TableInit = function () {
    ...
    oTableInit.Init = function () {
        ...
        $('#tb_departments').bootstrapTable({
            url: basePath+'/bk/l/sys/userList',  //请求后台的URL（*）       
            method: 'get',                      //请求方式（*）
                    ...
                    $.each(result,function(index,content){
                       ...
                       var s = '<div class="table-o-tooltip">';
							...
							if(isTrue==1){
								...
								content["enbole"] = "<span style='color:#f00'>否</span>";
							...
							s += '<button type="button" class="btn btn-danger btn-sm" style="margin-right:5px;" onclick="deleteUser('+content["id"]+');" >删除</button>';	
							s += '</div>';
							content['use'] = s;
                    });
...
function update(id){
	retFrom();
	console.log(id);
	button_submit = "editUser";
	tmUtil.request({
		url:basePath+"/bk/l/sys/getUserById",
		data:{
			"id":id
		},
		success:function(data){
...
function updateState(id,state){
	tmUtil.request({
		url:basePath+"/bk/l/sys/editUserState",
		data:{
			"id":id,
			"isTrue":state
		},
